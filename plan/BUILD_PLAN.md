...
| R8 | **"Never injured" and "never checked" are not distinguishable** | a player with no injury rows reads as a clean record. **Re-scoped by Decision 12** (spec, 14 Aug): Medical is no longer scored automatically, so this no longer corrupts a score — but the availability figure is still *displayed to scouts as evidence*, and a blank record shown as "100% available" would mislead the human making the judgement. Partial resolution exists: **minutes played is an independent check** — a player with 2,000+ minutes was demonstrably available whatever Transfermarkt says, and that resolves ~37% of blank records at a rate consistent across leagues (34/39/37/36%). The remainder must be **labelled "not known", never "clean"**. Blocking on the evidence panel, not on scoring |
| R9 | **Overlapping injury spells are double-counted** | Transfermarkt lists concurrent diagnoses as separate rows. Charlie Wyke carries "Ankle injury" **and** "Broken leg", both 26 Oct 2024 → 30 Jan 2026, 462 days and 64 matches **each** — so he reads as **128 matches missed against an actual 64**. Affects **73 spells across 54 players (5% of those with an injury record)** and hits hardest in exactly the severe cases where concurrent diagnoses occur, i.e. where the evidence matters most. Fix: **merge overlapping date ranges before summing**, rather than adding rows. **Must be fixed before this evidence is shown to scouts** |
